...
contract Mineral420 is ERC721Enumerable, ERC721Burnable, Ownable {
    using Strings for uint256;

    string private baseTokenURI;

    constructor(string memory baseTokenURI_) ERC721("Mineral 420", "Mineral") {
    ...
    function tokenURI(
        ...
        require(
            _tokenExists(tokenId),
            "World : URI query for nonexistent token"
        );
    ...
    function batchMint(address user, uint256 amount) public onlyOwner {
        uint256[] memory tokenIds = new uint256[](amount);
        uint256 supply = totalSupply();
        for (uint256 index = 0; index < amount; index++) {
            uint256 tokenId = supply + index;
            _mint(user, tokenId);

            tokenIds[index] = tokenId;
        }
        emit BatchMint(user, tokenIds);
    }
    ...
    event BatchMint(address indexed user, uint256[] tokenIds);
    event Mint(
